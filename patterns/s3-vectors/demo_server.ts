...
app.get('/search/:rowIndex', async (req, res) => {
  ...
  try {
    ...
    const getResult = await client.send(new GetVectorsCommand({
      vectorBucketName: VECTOR_BUCKET_NAME,
      indexName: INDEX_NAME,
      keys: [rowIndex],
      returnData: true,
    ...
    const queryResult = await client.send(new QueryVectorsCommand({
      vectorBucketName: VECTOR_BUCKET_NAME,
      indexName: INDEX_NAME,
...
app.delete('/vectors', async (_req, res) => {
  try {
    ...
    do {
      const listResult = await client.send(new ListVectorsCommand({
        vectorBucketName: VECTOR_BUCKET_NAME,
        indexName: INDEX_NAME,
        maxResults: 500,
        nextToken,
