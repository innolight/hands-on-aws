...
describe('PUT /index', () => {
  test('creates index and returns {created}', async () => {
    ...
    expect(indicesCreate).toHaveBeenCalledWith(
...
describe('POST /products', () => {
  test('indexes product and returns generated id', async () => {
    ...
    expect(index).toHaveBeenCalledWith(
      expect.objectContaining({index: INDEX_NAME, id: PRODUCT.id, body: PRODUCT})
    );
  });
...
describe('POST /products/_bulk', () => {
  ...
  test('reports partial failures from bulk response', async () => {
    const bulk = jest.fn().mockResolvedValue(okResponse({
      errors: true,
      items: [
    ...
    const res = await request(app).post('/products/_bulk')
...
describe('GET /products/:id', () => {
  test('returns product by id', async () => {
    const get = jest.fn().mockResolvedValue(okResponse({
      _id: 'prod-1',
...
describe('GET /search', () => {
  ...
  test('returns hits and next_search_after cursor', async () => {
    const search = jest.fn().mockResolvedValue(makeSearchResponse([
      {_id: 'prod-1', _source: {name: 'Headphones'}, sort: [1.5, 'prod-1']},
    ]));
    const app = createApp(makeClient({search}));
...
describe('withRetry', () => {
  ...
  test('throws after exhausting all attempts on repeated 429', async () => {
    ...
    let caught: unknown;
    const promise = withRetry(fn, 3).catch(e => { caught = e; });
    await jest.runAllTimersAsync();
    await promise;
