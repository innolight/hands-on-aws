...
export const ec2sAlbNetworkingStackName = 'Ec2sAlbNetworkingStack';

interface Ec2sAlbNetworkingStackProps extends cdk.StackProps {
  vpc: ec2.Vpc;
}

// Internet → ALB → listener rules → per-service target groups → EC2 instances (Docker)
export class Ec2sAlbNetworkingStack extends cdk.Stack {
  public readonly listener: elbv2.ApplicationListener;

  constructor(scope: Construct, id: string, props: Ec2sAlbNetworkingStackProps) {
    ...
    // Each service adds its own ListenerRule with a path condition (e.g. /ec2s-alb/*).
