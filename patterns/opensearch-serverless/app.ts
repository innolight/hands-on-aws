...
// Retry wrapper for 429 (OCU throttling). The transport layer retries 502/503/504 but
// NOT 429 — those indicate the collection has hit its OCU limit and needs time to scale.
...
export async function withRetry<T>(fn: () => Promise<T>, maxAttempts = 4): Promise<T> {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(
        `OCU throttle (429), attempt ${attempt}/${maxAttempts}, retrying in ${Math.round(backoff + jitter)}ms`,
      );
...
export function createApp(client: Client): express.Application {
    ...
    try {
      await withRetry(() =>
        client.indices.create({
          ...
          body: {
            settings: {
              // AOSS manages number_of_shards, number_of_replicas, and refresh_interval
              // automatically. Specifying them here causes a 400 error.
            },
            mappings: {
  ...
  // Note: newly indexed documents are not immediately searchable. AOSS refreshes
  // automatically but typically with ~10s delay (refresh_interval is managed by AWS).
  ...
  app.post('/products/_bulk', async (req, res) => {
    ...
    try {
      const body = products.flatMap((product) => [{ index: { _index: INDEX_NAME, _id: product.id } }, product]);
  ...
  // (Scroll API is not supported in AOSS).
...
function handleError(res: express.Response, err: unknown) {
  ...
  if (err instanceof errors.ResponseError) {
    ...
    if (status === 404) {
      ...
      res.status(429).json({ error: 'OCU throttle — retry after backoff', detail: err.message });
    } else {
